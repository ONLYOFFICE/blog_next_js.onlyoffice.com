import StyledAuthorContent from "./styled-author-content";
import Breadcrumbs from "@components/screens/common/breadcrumbs";
import Heading from "@components/common/heading";
import LoadMorePosts from "@components/screens/common/load-more-posts";

const AuthorContent = ({ t, locale, posts, isAuthorContent, authorName, authorSlug }) => {
  return (
    <StyledAuthorContent>
      <Breadcrumbs className="breadcrumbs" t={t} data={posts} isAuthorContent={isAuthorContent} />

      <Heading className="author-title" level={2}>
        {`${t("Author Archives")}: `}
        <span className="author-name">{authorName}</span>
      </Heading>

      <div className="author-posts">
        <LoadMorePosts t={t} locale={locale} data={posts} isAuthorContent={isAuthorContent} authorSlug={authorSlug} />
      </div>
    </StyledAuthorContent>
  );
};

export default AuthorContent;
