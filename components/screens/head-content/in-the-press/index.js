...
const InThePressHeadSEO = ({ title, metaSiteName, currentLanguage, articlePublishedTime, articleModifiedTime }) => {
  const baseUrl = `https://www.onlyoffice.com/blog/${currentLanguage === "en" ? "" : `${currentLanguage}/`}onlyoffice-in-the-press/`;
  const languagesKey = 
  ...
  return (
    <Head>
      ...
      {languages.map((lng) => {
        ...
        return (
          <link key={key} rel="alternate" hrefLang={key} href={`https://www.onlyoffice.com/blog/${shortKey === "en" ? "" : `${shortKey}/`}onlyoffice-in-the-press/`} />
        );
      })}
