// Cheap pre-filter for routes with `fallback: "blocking"`:
// pages/[...post].js and the tag / author / category [slug].js routes.
//
// These routes turn any path that isn't in the pre-generated list into a
// GraphQL query against WordPress - each unique URI is a cache miss that runs
// a heavy WPML JOIN. External bots hammer non-existent paths
// (<real-post>/redirect.php, traversal probes, fuzzing polyglots with quotes),
// and every one of those reaches WP.
//
...
// path traversal, control chars, quotes). Anything that looks like a plausible
// slug - including posts created after the last build and non-latin
...
// Checked against BOTH the raw path and its percent-decoded form, so encoded
// and mixed-encoded variants (".%2e", "%252e%252e") are caught as well.
const GARBAGE_MARKERS = [
  ...
  "..",       // path traversal
  "%2e%2e",   // encoded ".." (catches double-encoded traversal after one decode)
  "%5c",      // encoded backslash (same)
  "%00",      // null byte (same)
  "%2f%2f",   // encoded "//" (same)
  "<", ">",   // angle brackets - XSS/scan noise, impossible in a slug
  "\"", "'",  // quotes - fuzzing polyglots; sanitize_title strips them
  "`",        // backtick - same
  "@",        // at sign - scanner reflection markers ("915801@"), stripped by sanitize_title
];
...
/**
 * Returns true if `uri` cannot be a real post/term and must be short-circuited
 * to a 404 without querying WordPress.
 *
 * @param {string} uri - the joined catch-all path (e.g. "2026/07/my-post") or
 *                       a single term slug for tag/author/category routes
 * @returns {boolean}
 */
function isGarbagePath(uri) {
  ...
  // Percent-decode once so markers hidden behind encoding (".%2e/", "%22")
  // are visible. Legit non-latin slugs decode cleanly; malformed encoding
  // never matches a real slug, so it is garbage by definition.
  let decoded;
  try {
    decoded = decodeURIComponent(lower).toLowerCase();
  } catch {
    return true;
  }
  ...
  if (GARBAGE_MARKERS.some((marker) => lower.includes(marker) || decoded.includes(marker))) {
    return true;
  }
