...
export async function getMainPostExcerpt(locale) {
  const data = await fetchAPI(`
    {
...
export async function getPostsUri(locale) {
  const data = await fetchAPI(`
    {
      posts(first: 1100, where: { wpmlLanguage: "${locale}" }) {
        edges {
          node {
            uri
          }
...
export async function getAuthorSlug(locale) {
  const data = await fetchAPI(`
    {
      posts(first: 100, where: { wpmlLanguage: "${locale}" }) {
        edges {
          node {
            author {
              node {
...
export async function getSearchResults(locale, first, after, query) {
  const data = await fetchAPI(`
    {
      posts(first: ${first}, after: ${after}, where: { wpmlLanguage: "${locale}", search: "${query}", orderby: { field: DATE, order: DESC }}) {
        edges {
          node {
            id
            title
...
export async function getLastPosts(locale) {
  const data = await fetchAPI(`
    {
      posts(first: 3, where: { wpmlLanguage: "${locale}" }) {
        edges {
...
export async function getRevalidatePost(locale, uri) {
  const data = await fetchAPI(`
    {
      posts(first: 1, where: { wpmlLanguage: "${locale}" }) {
        edges {
