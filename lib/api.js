...
export async function getInThePressPosts(locale, first, after) {
  const data = await fetchAPI(`
    {
      news(first: ${first}, after: ${after} where: {wpmlLanguage: "${locale}", orderby: {order: DESC, field: META}, metaQuery: {metaArray: {type: NUMERIC, key: "dateNews"}}}) {
        edges {
          node {
            id
            title
...
export async function getPostsUri(locale) {
  const data = await fetchAPI(`
    {
      posts(first: 600, where: { wpmlLanguage: "${locale}" }) {
        edges {
          node {
            uri
          }
...
export async function getCategorySlug(locale) {
  const data = await fetchAPI(`
    {
      categories(first: 600, where: { wpmlLanguage: "${locale}" }) {
        edges {
          node {
            slug
          }
